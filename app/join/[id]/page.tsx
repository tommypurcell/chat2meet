...
export default function JoinPage({
  ...
  const displayTime = slot?.time || `${event?.durationMinutes || 30} min`;
      ...
      {event && !error && (
        <Card className="mb-6 w-full max-w-md">
          <CardContent className="flex items-center gap-3 py-4">
            <Avatar name={event.createdBy} size={36} />
            <div className="min-w-0 flex-1">
              <p className="text-sm text-[var(--text-secondary)]">
                <span className="font-semibold text-[var(--text-primary)]">{event.createdBy}</span>
                {" "}invited you to:
              </p>
      ...
      <Card className="w-full max-w-md">
        <CardContent className="space-y-5 px-6 py-6">
          ...
          <div className="space-y-3">
            <div>
              <label htmlFor="join-name" className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Name</label>
              <input
                id="join-name"
                type="text"
                value={name}
            ...
            <div>
              <label htmlFor="join-email" className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Email</label>
              <input
                id="join-email"
                type="email"
                value={email}
            ...
            <div>
              <label htmlFor="join-password" className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Password</label>
              <input
                id="join-password"
                type="password"
                value={password}
          ...
          <p className="text-center text-sm text-[var(--text-tertiary)]">
            ...
            <a href="/login" className="font-medium text-[var(--text-link)] hover:underline">
              Sign in
            </a>
          </p>
        </CardContent>
